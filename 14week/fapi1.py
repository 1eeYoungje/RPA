...
async def generate_qr(data: str = Form(...)):
    ...
    #text칸에 입력하면 입력한 값이 datastr로 들어가고 다시 formdata로 들어가고 
    # qrcode로 들어가면 파이썬 파일에서 qr.make에서 만들어진 후 stramingRespose(~~_arr)로 이미지를 배열에 담아 다시 전송되고 qrcodeImg라는 칸에 출력된다.
